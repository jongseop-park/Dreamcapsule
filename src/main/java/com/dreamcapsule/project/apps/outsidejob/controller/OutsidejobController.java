...
public class OutsidejobController {
    ...
    public void holidayExcelDown(HttpServletResponse response) throws Exception {
        ...
        for (int x = 0; x < outsideJobList.size();x++){
            row = sheet.createRow(rowNo++);
            int seq = outsideJobList.get(x).getUserNum().intValue()-1;
            int y = 0;
            cell = row.createCell(y++);
            cell.setCellValue(outsideJobInfo.get(seq).getName());
            cell = row.createCell(y++);
            cell.setCellValue(outsideJobInfo.get(seq).getTask());
            cell = row.createCell(y++);
            cell.setCellValue(outsideJobInfo.get(seq).getJobGrade());
            cell = row.createCell(y++);
            cell.setCellValue(outsideJobList.get(x).getMonth() + " (총 " + outsideJobList.get(x).getDay() + "일)");
            cell = row.createCell(y++);
            cell.setCellValue(outsideJobList.get(x).getStartTime() + " ~ " + outsideJobList.get(x).getEndTime());
            cell = row.createCell(y++);
            cell.setCellValue(outsideJobList.get(x).getStartPlace());
            cell = row.createCell(y++);
            cell.setCellValue(outsideJobList.get(x).getEndPlace());
            cell = row.createCell(y);
